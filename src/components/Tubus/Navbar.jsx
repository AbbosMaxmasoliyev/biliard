...
import linksSvetilnik from "../../json/Links/keyChexliTubus.json";
        ...
        if (name == params.category) {
            ...
            <div key={index}>
              ...
              {linksSvetilnik[name].links.map((item) => (
                <Link
                  ...
                  to={`/tubus/${name}/${item.link.replace("/", "")}`}
                >
                  <p>{item.name}</p>
                </Link>
              ))}
          ...
          return (
            <span className="h3" key={index}> 
              <Link to={`/tubus/${name}`}>{linksSvetilnik[name].name}</Link>
            </span>
          );
        }
      })}
