...
const SvetilnikCategoryComponent = () => {
  const params = useParams();
  const [links, setLinks] = useState();
  const [obj, setObj] = useState();
  useEffect(() => {
    let category = params.category;
    console.log(keyLinksJson[category]);
    setLinks(
      (prev) => prev = keyLinksJson[category]?.items.map((item) => item.link)
    );
          ...
          <ul className="related-products tables">
            ...
            console.log(obj[item]);
              return (
                <li key={index}>
                  <div>
                    <Link to={`/sukno/${params.category}/${item}`} title={obj[item]?.name}>
                      <img src={obj[item]?.images[0]} className="w178" />
                    </Link>
                  </div>
                  <div
                    style={{
...
export default SvetilnikCategoryComponent;
